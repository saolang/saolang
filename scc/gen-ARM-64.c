...
{
    scc_assert_macro(TREG_R(0) <= r && r <= TREG_R30);
    return r < TREG_R30 ? r : 30;
}

static uint32_t fltr(int r)
{
    scc_assert_macro(TREG_F(0) <= r && r <= TREG_F(7));
    return r - TREG_F(0);
}
...
static void arm64_ldrs(int reg_, int size)
{
    ...
    switch (size) {
    default: scc_assert_macro(0); break;
    case 1:
...
{
    // Currently SCC's linker does not generate COPY relocations for
    // STT_OBJECTs when scc is invoked with "-run". This typically
...
static unsigned long arm64_pcs(int n, CType **type, unsigned long *a)
{
    ...
    if ((type[0]->t & VT_BTYPE) == VT_VOID)
        ...
        scc_assert_macro(a[0] == 0 || a[0] == 1 || a[0] == 16);
    }
    ...
    if (0) {
        ...
        for (i = 0; i <= n; i++) {
            if (!i)
                SCC(printf)("arm64_pcs return: ");
            else
                SCC(printf)("arm64_pcs arg %d: ", i);
            if (a[i] == (unsigned long)-1)
                SCC(printf)("void\n");
            else if (a[i] == 1 && !i)
                SCC(printf)("X8 pointer\n");
            else if (a[i] < 16)
                SCC(printf)("X%lu%s\n", a[i] / 2, a[i] & 1 ? " pointer" : "");
            else if (a[i] < 32)
                SCC(printf)("V%lu\n", a[i] / 2 - 8);
            else
                SCC(printf)("stack %lu%s\n",
...
ST_FUNC void gfunc_call(int nb_args)
{
    ...
    t = scc_malloc((nb_args + 1) * sizeof(*t));
    a = scc_malloc((nb_args + 1) * sizeof(*a));
    a1 = scc_malloc((nb_args + 1) * sizeof(*a1));
    ...
    for (i = nb_args; i; i--)
        if (a[i] & 1) {
            ...
            scc_assert_macro((arg->type.t & VT_BTYPE) == VT_STRUCT);
...
ST_FUNC void gfunc_prolog(CType *func_type)
{
    ...
    for (sym = func_type->ref; sym; sym = sym->next)
        ++n;
    t = scc_malloc(n * sizeof(*t));
    a = scc_malloc(n * sizeof(*a));
...
ST_FUNC void gen_va_arg(CType *t)
{
    ...
    if (!hfa) {
        ...
        if (align == 16) {
            scc_assert_macro(0); // this path untested but needed for __uint128_t
...
{
    scc_assert_macro(a - ind + 0x8000000 < 0x10000000);
...
static void arm64_gen_opil(int op, uint32_t l)
{
    ...
    gv2(RC_INT, RC_INT);
    scc_assert_macro(vtop[-1].r < VT_CONST && vtop[0].r < VT_CONST);
    a = intr(vtop[-1].r);
    b = intr(vtop[0].r);
    vtop -= 2;
    x = get_reg(RC_INT);
...
{
    int f = vtop[0].type.t;
    scc_assert_macro(t == VT_FLOAT || t == VT_DOUBLE || t == VT_LDOUBLE);
    scc_assert_macro(f == VT_FLOAT || f == VT_DOUBLE || f == VT_LDOUBLE);
    if (t == f)
        return;
    ...
    if (t == VT_LDOUBLE || f == VT_LDOUBLE) {
        ...
        int x, a;
        gv(RC_FLOAT);
        scc_assert_macro(vtop[0].r < VT_CONST);
        a = fltr(vtop[0].r);
        --vtop;
        x = get_reg(RC_FLOAT);
        ++vtop;
