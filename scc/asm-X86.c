/*
 *  i386 specific functions for SCC assembler
 *
...
//static inline int get_reg_shift(SCCState *s1)
//{
//    int shift;
...
static void gen_disp32(ExprValue *pe)
{
    ...
    if (esym && esym->st_shndx == cur_text_section->sh_num) {
        /* same section: we can output an absolute value. Note
           that the SCC compiler behaves differently here because
...
static void maybe_print_stats (void)
{
  ...
  if (!already)
	...
	already = 1;
        ...
        SCC(printf)("size=%d nb=%d f0=%d f1=%d f2=%d f3=%d\n",
...
//@ref sccgen.c:decl_initializer_alloc()
ST_FUNC int asm_parse_regvar(int t)
{
    ...
    if (s[0] != '%')
        return -1;
    t = tok_alloc(s+1, SCC(strlen,int)(s)-1)->tok;
    unget_tok(t);
    unget_tok('%');
                                    ...
                                    int *pout_reg)
{
    ...
    *pout_reg = -1;
    ...
    for(i=0;i<nb_operands;i++) {
        j = sorted_op[i];
        op = &operands[j];
        SCC(printf)("%%%d [%s]: \"%s\" r=0x%04x reg=%d\n",
               j,
                              ...
                              SValue *sv, int modifier)
{
    ...
    if ((r & VT_VALMASK) == VT_CONST) {
		...
		get_asm_sym(tok_alloc(name, SCC(strlen,int)(name))->tok, sv->sym);
	    }
    ...
    no_offset:;
        ...
        SCC(snprintf)(buf, sizeof(buf), "%%%s", get_tok_str(reg, NULL));
        cstr_cat(add_str, buf, -1);
    }
}
                         ...
                         int out_reg)
{
	...
	SCC(memcpy)(regs_allocated, clobber_regs, sizeof(regs_allocated));
	for(i = 0; i < nb_operands;i++) {
		op = &operands[i];
		if (op->reg >= 0)
...
ST_FUNC void asm_clobber(uint8_t *clobber_regs, const char *str)
{
	...
	if (!SCC(strcmp,int)(str, "memory") ||
			!SCC(strcmp,int)(str, "cc") ||
			!SCC(strcmp,int)(str, "flags"))
		return;
	ts = tok_alloc(str, SCC(strlen,int)(str));
	reg = ts->tok;
