...
#if defined(SCC_ARM_EABI) && !defined(SCC_ARM_VFP)
#error "Currently SaoCC only supports float computation with VFP instructions"
#endif
...
#ifndef SCC_CPU_VERSION
# define SCC_CPU_VERSION 5
#endif
...
#ifndef SCC_ARM_HARDFLOAT
    scc_warning("soft float ABI currently not supported: default to softfp");
#endif
}
#else
...
{
#if 0
#if !defined (SCC_ARM_VFP)
    scc_warning("Support for FPA is deprecated and will be removed in next"
                " release");
#endif
#if !defined (SCC_ARM_EABI)
    scc_warning("Support for OABI is deprecated and will be removed in next"
                " release");
#endif
#endif
}
...
//const char *default_elfinterp(struct SCCState *s)
//{
...
void o(uint32_t i)
{
  ...
  ind1 = ind + 4;
  if (!cur_text_section)
    scc_error("compiler error! This happens f.ex. if the compiler\n"
...
uint32_t encbranch(int pos, int addr, int fail)
{
  ...
  if(addr>=0x1000000 || addr<-0x1000000) {
    if(fail)
      scc_error("FIXME: function bigger than 32MB");
    return 0;
  }
...
static uint32_t vfpr(int r)
{
  if(r<TREG_F0 || r>TREG_F7)
    scc_error("compiler error! register %i is no vfp register",r);
  return r - TREG_F0;
}
#else
static uint32_t fpr(int r)
{
  if(r<TREG_F0 || r>TREG_F3)
    scc_error("compiler error! register %i is no fpa register",r);
  return r - TREG_F0;
}
#endif
...
static uint32_t intr(int r)
{
  ...
  if (r >= TREG_SP && r <= TREG_LR)
    return r + (13 - TREG_SP);
  scc_error("compiler error! register %i is no int register",r);
	return -1;
}
...
static uint32_t mapcc(int cc)
{
  ...
  scc_error("unexpected condition code");
...
/* Assign parameters to registers and stack with alignment according to the
   ...
   Note: this function allocated an array in plan->pplans with scc_malloc. It
...
static int assign_regs(int nb_args, int float_abi, struct plan *plan, int *todo)
{
  ...
  ncrn = nsaa = 0;
  *todo = 0;
  plan->pplans = scc_malloc(nb_args * sizeof(*plan->pplans));
  SCC(memset)(plan->clsplans, 0, sizeof(plan->clsplans));
  for(i = nb_args; i-- ;) {
...
again:
  ...
  /* second pass to restore registers that were saved on stack by accident.
     Maybe redundant after the "lvalue_save" patch in sccgen.c:gv() */
  if (++pass < 2)
    goto again;
...
{
    if ((bytes & 3))
      scc_error("alignment of code section not multiple of 4");
    while (bytes > 0) {
	o(0xE1A00000);
	bytes -= 4;
    }
...
void gen_opi(int op)
{
  ...
  switch(c) {
    ...
    case 3:
      ...
      break;
    default:
      scc_error("gen_opi %i unimplemented!",op);
  }
}
...
/* bug (intention?) in Linux FPU emulator
   doesn't set carry if equal */
	switch(op) {
	  ...
	  case TOK_ULE:
	  case TOK_UGT:
            scc_error("unsigned comparison on floats?");
	    break;
	  case TOK_LT:
            op=TOK_Nset;
	    break;
...
ST_FUNC void gen_cvt_itof1(int t)
{
  ...
  scc_error("unimplemented gen_cvt_itof %x!",vtop->type.t);
}
...
void gen_cvt_ftoi(int t)
{
  ...
  scc_error("unimplemented gen_cvt_ftoi!");
}
...
#endif
    if (align & (align - 1))
        scc_error("alignment is not a power of 2: %i", align);
