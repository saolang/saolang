...
#define tal_realloc(al, p, size) scc_realloc(p, size)
...
tail_call:
    if (!al)
        return;
#ifdef TAL_INFO
    SCC(fprintf)(SCCSTD(err), "limit=%5d, size=%5g MB, nb_peak=%6d, nb_total=%8d, nb_missed=%6d, usage=%5.1f%%\n",
    ...
    if (al->nb_allocs > 0) {
        uint8_t *p;
        SCC(fprintf)(SCCSTD(err), "TAL_DEBUG: memory leak %d chunk(s) (limit= %d)\n",
                al->nb_allocs, al->limit);
        p = al->buffer;
        while (p < al->p) {
            tal_header_t *header = (tal_header_t *)p;
            if (header->line_num > 0) {
                SCC(fprintf)(SCCSTD(err), "%s:%d: chunk of %d bytes leaked\n",
...
tail_call:
    if (al->buffer <= (uint8_t *)p && (uint8_t *)p < al->buffer + al->size) {
        ...
        if (header->line_num < 0) {
            SCC(fprintf)(SCCSTD(err), "%s:%d: TAL_DEBUG: double frees chunk from\n",
                    file, line);
            SCC(fprintf)(SCCSTD(err), "%s:%d: %d bytes\n",
...
ST_FUNC void cstr_cat(CString *cstr, const char *str, int len)
{
    int size;
    if (len <= 0)
        len = SCC(strlen,int)(str) + 1 + len;
    size = cstr->size + len;
    if (size > cstr->size_allocated)
        cstr_realloc(cstr, size);
    SCC(memmove)(((unsigned char *)cstr->data) + cstr->size, str, len);
    cstr->size = size;
}
...
{
    SCC(memset)(cstr, 0, sizeof(CString));
}
...
ST_FUNC const char *get_tok_str(int v, CValue *cv)
{
...
#ifdef _WIN32
        SCC(sprintf)(p, "%u", (unsigned)cv->i);
#else
        SCC(sprintf)(p, "%llu", (unsigned long long)cv->i);
#endif
        break;
    case TOK_LCHAR:
    ...
    default:
        ...
        if (v >= 127) {
            SCC(sprintf)(cstr_buf.data, "<%02x>", v);
            return cstr_buf.data;
        }
        addv:
            *p++ = v;
            *p = '\0';
            ...
            SCC(sprintf)(p, "L.%u", v - SYM_FIRST_ANOM);
        } else {
            /* should never happen */
            return NULL;
        }
...
ST_FUNC int handle_eob(void)
{
    ...
    if (bf->buf_ptr >= bf->buf_end) {
        if (bf->fd >= 0) {
            ...
            len = IO_BUF_SIZE;
#endif
            len = SCC(read,int)(bf->fd, bf->buffer, len);
            if (len < 0)
                len = 0;
        } else {
            len = 0;
...
ST_FUNC int *tok_str_dup(TokenString *s)
{
    ...
    SCC(memcpy)(str, s->str, s->len * sizeof(int));
    return str;
}
    ...
    case TOK_LSTR:
        {
            ...
            str[len] = cv->str.size;
            SCC(memcpy)(&str[len + 1], cv->str.data, cv->str.size);
            len += nb_words;
        }
        break;
    case TOK_CDOUBLE:
...
static int macro_is_equal(const int *a, const int *b)
{
    ...
    while (*a && *b) {
        ...
        TOK_GET(&t, &b, &cv);
        if (SCC(strcmp,int)(macro_equal_buf.data, get_tok_str(t, &cv)))
            return 0;
    }
    return !(*a || *b);
}
...
ST_INLN void define_push(int v, int macro_type, int *str, Sym *first_arg)
{
	...
	scc_warning("%s redefined", get_tok_str(v, NULL));
}
...
ST_FUNC void label_pop(Sym **ptop, Sym *slast, int keep)
{
    ...
    for(s = *ptop; s != slast; s = s1) {
        ...
        if (s->r == LABEL_DECLARED) {
            scc_warning("label '%s' declared but not used", get_tok_str(s->v, NULL));
...
/* fake the nth "#if defined test_..." for scc -dt -run */
...
static CachedInclude *search_cached_include(SCCState *s1, const char *filename, int add)
{
    ...
    e = scc_malloc(sizeof(CachedInclude) + SCC(strlen,int)(filename));
    SCC(strcpy)(e->filename, filename);
...
#ifdef INC_DEBUG
					SCC(printf)("%s: skipping cached %s\n", file->filename, buf1);
#endif
					goto include_done;
				}
...
test_skip:
		...
		case TOK_ENDIF:
			...
			break;
		case TOK_PPNUM:
			n = SCC(strtoul,int)((char*)tokc.str.data, &q, 10);
			goto _line_num;
		case TOK_LINE:
			next();
			if (tok != TOK_CINT)
    ...
    if (ch == '.' ||
        ...
        if (b != 10) {
            ...
            *q = '\0';
            ...
            t = toup(ch);
            d = SCC(ldexp,double)(d, exp_val - frac_bits);
            if (t == 'F') {
                ch = *p++;
                tok = TOK_CFLOAT;
...
#else
                tok = TOK_CLDOUBLE;
                tokc.ld = SCC(strtold,long double)(token_buf, NULL);
#endif
            } else {
                tok = TOK_CDOUBLE;
                tokc.d = SCC(strtod,double)(token_buf, NULL);
    ...
    parse_ident_fast:
        ...
        if (c != '\\') {
            ...
            for(;;) {
                ...
                if (!ts)
                    break;
                if (ts->len == len && !SCC(memcmp,int)(ts->str, p1, len))
                    goto token_found;
                pts = &(ts->hash_next);
            }
    ...
    Sym *s)
{
    ...
    if (tok == TOK___LINE__ || tok == TOK___COUNTER__) {
        ...
        SCC(snprintf)(buf, sizeof(buf), "%d", t);
        cstrval = buf;
        t1 = TOK_PPNUM;
        goto add_cstr1;
        ...
        tm = SCC(localtime)(&ti);
        if (tok == TOK___DATE__) {
            SCC(snprintf)(buf, sizeof(buf), "%s %2d %d", 
                     ab_month_name[tm->tm_mon], tm->tm_mday, tm->tm_year + 1900);
        } else {
            SCC(snprintf)(buf, sizeof(buf), "%02d:%02d:%02d", 
...
ST_FUNC void sccpp_new(SCCState *s)
{
    ...
    tal_new(&cstr_alloc, CSTR_TAL_LIMIT, CSTR_TAL_SIZE);
		...
		hash_ident = SCC(malloc,TokenSym**)(TOK_HASH_SIZE * sizeof(TokenSym *));//tmp
    SCC(memset)(hash_ident, 0, TOK_HASH_SIZE * sizeof(TokenSym *));
    cstr_new(&cstr_buf);
...
static void tok_print(const char *msg, const int *str)
{
	...
	fp = scc_state->ppfp;
	SCC(fprintf)(fp, "%s", msg);
	while (str) {
		TOK_GET(&t, &str, &cval);
		if (!t)
			break;
		SCC(fprintf)(fp, " %s%d", get_tok_str(t, &cval), s), s = 1;
	}
	SCC(fprintf)(fp, "\n");
}
...
static void define_print(SCCState *s1, int v)
{
    ...
    fp = s1->ppfp;
    SCC(fprintf)(fp, "#define %s", get_tok_str(v, NULL));
    if (s->type.t == MACRO_FUNC) {
        Sym *a = s->next;
        SCC(fprintf)(fp,"(");
        if (a)
            for (;;) {
                SCC(fprintf)(fp,"%s", get_tok_str(a->v & ~SYM_FIELD, NULL));
                if (!(a = a->next))
                    break;
                SCC(fprintf)(fp,",");
            }
...
static void pp_debug_defines(SCCState *s1)
{
    ...
    if (t == TOK_DEFINE) {
        ...
        SCC(fprintf)(fp, "#undef %s\n", vs);
    } else if (t == TOK_push_macro) {
        SCC(fprintf)(fp, "#pragma push_macro(\"%s\")\n", vs);
    } else if (t == TOK_pop_macro) {
        SCC(fprintf)(fp, "#pragma pop_macro(\"%s\")\n", vs);
    }
    pp_debug_tok = 0;
}
