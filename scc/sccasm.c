...
//NOTES: entry = scc_assemble()

#include "scc.h"

ST_FUNC int asm_get_local_label_name(SCCState *s1, unsigned int n)
{
    char buf[64];
    TokenSym *ts;

    SCC(snprintf)(buf, sizeof(buf), "L..%u", n);
    ts = tok_alloc(buf, SCC(strlen,int)(buf));
    return ts->tok;
}
...
static Sym* asm_section_sym(SCCState *s1, Section *sec)
{
    ...
    int label = tok_alloc(buf,
        SCC(snprintf,int)(buf, sizeof buf, "L.%s", sec->name)
        )->tok;
...
static void asm_expr_unary(SCCState *s1, ExprValue *pe)
{
    ...
    case TOK_PPNUM:
        p = tokc.str.data;
        n = SCC(strtoull,unsigned long long)(p, (char **)&p, 0);
        if (*p == 'b' || *p == 'f') {
...
error_constant:
				...
				vl = SCC(strtoll,unsigned long long)(p, (char **)&p, 0);
				if (*p != '\0')
					goto error_constant;
				next();
				if (sec->sh_type != SHT_NOBITS) {
...
asm_data:
		...
		case TOK_ASMDIR_rept:
			{
				...
				while (repeat-- > 0) {
					scc_assemble_internal(s1, (parse_flags & PARSE_FLAG_PREPROCESS),
							global);
		...
		case TOK_ASMDIR_bss:
			{ 
				...
				if (n)
					SCC(sprintf)(sname, "%s%d", get_tok_str(tok1, NULL), n);
				else
					SCC(sprintf)(sname, "%s", get_tok_str(tok1, NULL));
		...
		case TOK_ASMDIR_size:
			{ 
				...
				if (!sym) {
					scc_error("label not found: %s", get_tok_str(tok, NULL));
				}

				/* XXX .size name,label2-label1 */
				if (s1->warn_unsupported)
					scc_warning("ignoring .size %s,*", get_tok_str(tok, NULL));
		...
		case TOK_ASMDIR_type:
			{ 
				...
				if (!SCC(strcmp,int)(newtype, "function") || !SCC(strcmp,int)(newtype, "STT_FUNC")) {
...
#endif
		default:
			scc_error("unknown assembler directive '.%s'", get_tok_str(tok, NULL));
			break;
	}
}//asm_parse_directive()
                               ...
                               int is_output)
{
    ...
    if (tok != ':') {
	    ...
	    parse_mult_str(&astr, "string constant");
            ...
            SCC(strcpy)(op->constraint, astr.data);
	    cstr_free(&astr);
            skip('(');
            gexpr();
            if (is_output) {
                ...
                if ((vtop->r & VT_LVAL) &&
                    ...
                    !SCC(strchr,char*)(op->constraint, 'm')) {
                    gv(RC_INT);
                }
            }
            op->vt = vtop;
...
ST_FUNC void asm_instr(void)
{
    ...
    nb_outputs = 0;
    must_subst = 0;
    SCC(memset)(clobber_regs, 0, sizeof(clobber_regs));
    if (tok == ':') {
        next();
        must_subst = 1;
        /* output args */
    ...
    /* assemble the string with scc internal assembler */
