...
{
#ifdef SCC_TARGET_PE
    scc_error("internal error: no GOT on PE: %s %x %x | %02x %02x %02x\n",
...
void load(int r, SValue *sv)
{
	...
	/* Like GCC we can load from small enough properly sized
	   ...
	   occur only with asm, so sccasm.c might also be a better place */
	if ((ft & VT_BTYPE) == VT_STRUCT) {
        ...
        if ((ft & VT_BTYPE) == VT_FLOAT) {
            ...
            scc_assert_macro(((ft & VT_BTYPE) == VT_INT)
		...
		o(0x037a + (REX_BASE(r) << 8));
            ...
            if ((r >= TREG_XMM0) && (r <= TREG_XMM7)) {
                if (v == TREG_ST0) {
                    ...
                    scc_assert_macro((v >= TREG_XMM0) && (v <= TREG_XMM7));
                    if ((ft & VT_BTYPE) == VT_FLOAT) {
                        o(0x100ff3);
                    } else {
                        scc_assert_macro((ft & VT_BTYPE) == VT_DOUBLE);
                        o(0x100ff2);
                    }
...
void gfunc_call(int nb_args)
{
	...
	switch (vtop->type.t & VT_BTYPE) {
	    ...
	    case VT_FLOAT:
	    case VT_DOUBLE:
		scc_assert_macro(mode == x86_64_mode_sse);
		r = gv(RC_FLOAT);
		o(0x50); /* push $rax */
		/* movq %xmmN, (%rsp) */
		o(0xd60f66);
	    ...
	    default:
		scc_assert_macro(mode == x86_64_mode_integer);
		/* simple type */
		/* XXX: implicit cast ? */
		r = gv(RC_INT);
    ...
    /* then, we prepare register passing arguments.
       ...
       instead of them */
    scc_assert_macro(gen_reg <= REGN);
    scc_assert_macro(sse_reg <= 8);
    for(i = 0; i < nb_args; i++) {
    ...
    scc_assert_macro(gen_reg == 0);
    scc_assert_macro(sse_reg == 0);
...
//@ref compare to the sccgen.c:gen_opl()
void gen_opi(int op)
{
    int r, fr, opc, c;
    int ll, uu, cc;
...
void gen_opf(int op)
{
    ...
    if ((vtop->type.t & VT_BTYPE) == VT_LDOUBLE) {
        ...
        if (op >= TOK_ULT && op <= TOK_GT) {
            ...
            scc_assert_macro(!(vtop[-1].r & VT_LVAL));
            ...
            scc_assert_macro((vtop->type.t & VT_BTYPE) != VT_LDOUBLE);
            switch(op) {
            default:
            case '+':
                a = 0;
            ...
            ft = vtop->type.t;
            fc = vtop->c.i;
            scc_assert_macro((ft & VT_BTYPE) != VT_LDOUBLE);
            ...
            scc_assert_macro(!(vtop[-1].r & VT_LVAL));
            if (swapped) {
                scc_assert_macro(vtop->r & VT_LVAL);
                gv(RC_FLOAT);
                vswap();
            }
