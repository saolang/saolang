...
static const char *pe_export_name(SCCState *s1, ElfW(Sym) *sym)
{
...
static int pe_find_import(SCCState * s1, ElfW(Sym) *sym)
{
    char buffer[200];
    const char *s, *p;
    int sym_index = 0, n = 0;
    ...
    do {
	    ...
	    if (sym->st_other & ST_PE_STDCALL) {
	        ...
	        p = SCC(strrchr,char*)(s, '@');
	        if (!p || s[0] != '_')
	            break;
	        SCC(strcpy,char*)(buffer, s+1)[p-s-1] = 0;
	    } else if (s[0] != '_') { /* try non-ansi function */
	        buffer[0] = '_', SCC(strcpy)(buffer + 1, s);
	    } else if (0 == SCC(memcmp,int)(s, "__imp_", 6)) { /* mingw 2.0 */
	        SCC(strcpy)(buffer, s + 6), a = 1;
	    } else if (0 == SCC(memcmp,int)(s, "_imp__", 6)) { /* mingw 3.7 */
	        SCC(strcpy)(buffer, s + 6), a = 1;
	    } else {
	        continue;
	    }
...
static int pe_fwrite(void *data, unsigned len, FILE *fp, DWORD *psum)
{
    ...
    return len == SCC(fwrite,unsigned)(data, 1, len, fp) ? 0 : -1;
}

static void pe_fpad(FILE *fp, DWORD new_pos)
{
    DWORD pos = SCC(ftell,DWORD)(fp);
    while (++pos <= new_pos)
        SCC(fputc)(0, fp);
}
...
#define CHARACTERISTICS_DLL 0x230F
    ...
    op = SCC(fopen,FILE*)(pe->filename, "wb");
    if (NULL == op) {
        scc_error_noabort("could not write '%s': %s", pe->filename, SCC(strerror,char*)(errno));
        return -1;
    }
    ...
    for (i = 0; i < pe->sec_count; ++i) {
        ...
        SCC(strncpy)((char*)psh->Name, sh_name, sizeof psh->Name);
    ...
    SCC(fseek)(op, offsetof(struct pe_header, opthdr.CheckSum), SEEK_SET);
    pe_fwrite(&pe_header.opthdr.CheckSum, sizeof pe_header.opthdr.CheckSum, op, NULL);
    SCC(fclose)(op);
#ifndef _WIN32
    SCC(chmod)(pe->filename, 0777);
#endif

    if (2 == pe->s1->verbose)
        SCC(printf)("-------------------------------\n");
    if (pe->s1->verbose)
        SCC(printf)("<- %s (%u bytes)\n", pe->filename, (unsigned)file_offset);
        ...
        for (k = 0, n = p->sym_count; k <= n; ++k) {
            if (k < n) {
                ...
                if (pe->type == PE_RUN) {
                    ...
                    if (!v)
                        scc_error_noabort("can't build symbol '%s'", name);
                } else
#endif
                if (ordinal) {
...
static void pe_build_exports(struct pe_info *pe)
{
    ...
    SCC(qsort)(sorted, sym_count, sizeof *sorted, sym_cmp);

    pe_align_section(pe->thunk, 16);
    dllname = scc_basename(pe->filename);
    ...
    SCC(strcpy)(scc_fileextension(buf), ".def");
    op = SCC(fopen,FILE*)(buf, "wb");
    if (NULL == op) {
        scc_error_noabort("could not create '%s': %s", buf, SCC(strerror,char*)(errno));
    } else {
...
static int pe_section_class(Section *s)
{
    ...
    if (flags & SHF_ALLOC) {
        if (type == SHT_PROGBITS) {
            ...
            if (flags & SHF_WRITE)
                return sec_data;
            if (0 == SCC(strcmp,int)(name, ".rsrc"))
                return sec_rsrc;
            if (0 == SCC(strcmp,int)(name, ".iedat"))
                return sec_idata;
            if (0 == SCC(strcmp,int)(name, ".pdata"))
                return sec_pdata;
            return sec_other;
        ...
        if (0 == SCC(strcmp,int)(name, ".reloc"))
            return sec_reloc;
        if (0 == SCC(strncmp,int)(name, ".stab", 5)) /* .stab and .stabstr */
            return sec_stab;
    }
    return -1;
}
...
static int pe_assign_addresses (struct pe_info *pe)
{
    ...
    section_order = scc_malloc(pe->s1->nb_sections * sizeof (int));
    ...
    pe->sec_info = scc_mallocz(o * sizeof (struct section_info));
    addr = pe->imagebase + 1;
    ...
    for (i = 1; i < pe->s1->nb_sections; ++i) {
        ...
        SCC(printf)("section %-16s %-10s %5x %s,%s,%s\n",
            s->name,
    ...
    for (sym_index = 1; sym_index < sym_end; ++sym_index) {
        ...
        if (sym->st_shndx == SHN_UNDEF) {
            ...
            if (type == STT_FUNC) {
                ...
                /* scc_realloc might have altered sym's address */
                sym = (ElfW(Sym) *)symtab_section->data + sym_index;
        ...
        not_found:
            if (ELFW(ST_BIND)(sym->st_info) == STB_WEAK)
                ...
                continue;
            scc_error_noabort("sccpe: undefined symbol '%s'%s", name,
...
static void pe_print_section(FILE * f, Section * s)
{
   ...
   SCC(fprintf)(f, "section  \"%s\"", s->name);
    if (s->link)
       SCC(fprintf)(f, "\nlink     \"%s\"", s->link->name);
    if (s->reloc)
   ...
   SCC(fprintf)(f, "%-8s", "offset");
    ...
    if (s->sh_type == SHT_SYMTAB || s->sh_type == SHT_RELX) {
        ...
        for (i = 0; p[i]; ++i)
           SCC(fprintf)(f, "%6s", p[i]);
       SCC(fprintf)(f, "  symbol");
    }
    ...
    for (i = 0; i < l;)
    {
       SCC(fprintf)(f, "%08X", i);
        ...
        if (s->sh_type == SHT_SYMTAB) {
           ...
           SCC(fprintf)(f, "  %04X  %04X  %04X   %02X    %02X    %02X   %04X  \"%s\"",
...
static void pe_print_sections(SCCState *s1, const char *fname)
{
    Section *s;
    FILE *f;
    int i;
    f = SCC(fopen,(FILE*))(fname, "w");
...
ST_FUNC SValue *pe_getimport(SValue *sv, SValue *v2)
{
    ...
    if (!sv->sym->a.dllimport)
        return sv;
    //SCC(printf)("import %04x %04x %04x %s\n", sv->type.t, sv->sym->type.t, sv->r, get_tok_str(sv->sym->v, NULL));
    SCC(memset)(v2, 0, sizeof *v2);
    v2->type.t = VT_PTR;
...
static int add_dllref(SCCState *s1, const char *dllname)
{
    DLLReference *dllref;
    int i;
    for (i = 0; i < s1->nb_loaded_dlls; ++i)
        if (0 == SCC(strcmp,int)(s1->loaded_dlls[i]->name, dllname))
            return i + 1;
    dllref = scc_mallocz(sizeof(DLLReference) + SCC(strlen,int)(dllname));
    SCC(strcpy)(dllref->name, dllname);
...
{
    SCC(lseek)(fd, offset, SEEK_SET);
    return len == SCC(read,int)(fd, buffer, len);
}
...
found:
    ...
    for (i = 0; i < ied.NumberOfNames; ++i) {
        ...
        for (l = 0;;) {
            if (n+1 >= n0)
                p = scc_realloc(p, n0 = n0 ? n0 * 2 : 256);
...
static int pe_load_res(SCCState *s1, int fd)
{
    ...
    if (hdr.filehdr.Machine != IMAGE_FILE_MACHINE
        ...
        || SCC(strcmp,int)((char*)hdr.sectionhdr.Name, ".rsrc") != 0)
        goto quit;
...
/* ------------------------------------------------------------- */
static int pe_load_def(SCCState *s1, int fd)
{
    int state = 0, ret = -1, dllindex = 0, ord;
    char line[400], dllname[80], *p, *x;
    FILE *fp;

    fp = SCC(fdopen,FILE*)(SCC(dup,int)(fd), "rb");
    while (SCC(fgets)(line, sizeof line, fp))
    {
        p = trimfront(trimback(line, SCC(strchr,char*)(line, 0)));
        if (0 == *p || ';' == *p)
            continue;

        switch (state) {
        case 0:
            if (0 != SCC(strnicmp,int)(p, "LIBRARY", 7))
                goto quit;
        ...
        case 1:
            if (0 != SCC(stricmp,int)(p, "EXPORTS"))
                goto quit;
            ++state;
            continue;
        ...
        default:
            ...
            x = SCC(strchr,char*)(p, ' ');
            if (x) {
                *x = 0, x = SCC(strrchr,char*)(x + 1, '@');
                if (x) {
                    char *d;
                    ord = SCC(strtol,int)(x + 1, &d, 10);
                    if (*d)
                        ord = 0;
                }
            }
...
/* ------------------------------------------------------------- */
static int pe_load_dll(SCCState *s1, const char *filename)
{
    char *p, *q;
    int index, ret;

    ret = scc_get_dllexports(filename, &p);
    if (ret) {
        return -1;
    } else if (p) {
        index = add_dllref(s1, scc_basename(filename));
        for (q = p; *q; q += 1 + SCC(strlen,int)(q))
...
ST_FUNC int pe_load_file(struct SCCState *s1, const char *filename, int fd)
{
    int ret = -1;
    char buf[10];
    if (0 == SCC(strcmp,int)(scc_fileextension(filename), ".def"))
        ret = pe_load_def(s1, fd);
    else if (pe_load_res(s1, fd) == 0)
        ret = 0;
    else if (read_mem(fd, 0, buf, 4) && 0 == SCC(memcmp,int)(buf, "MZ\220", 4))
...
{
    if (NULL == s1->uw_pdata) {
        s1->uw_pdata = find_section(scc_state, ".pdata");
    ...
    if (0 == s1->uw_offs) {
        ...
        SCC(memcpy)(p, uw_info, sizeof uw_info);
    }
...
//TODO
//ST_FUNC void sccpe_new(SCCState *s)
//{
//}
