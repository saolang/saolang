//@ref scc-__SCC_TARGET_FORMAT__ .c|.o
#include "scc.h"
...
ST_FUNC void scc_format_delete(SCCState *s1)
{
    int i;
    ...
    for ( i = 0; i < s1->nb_loaded_dlls; i++) {
        ...
        if ( ref->handle )
# ifdef _WIN32
            SCC(FreeLibrary)((HMODULE)ref->handle);
# else
            dlclose(ref->handle);
# endif
    }
    ...
    symtab_section = NULL; /* for sccrun.c:rt_printline() */
}
    ...
    for (i = 1; i < s1->nb_sections; i++) {
        ...
        if (sr->sh_type == SHT_RELX && sr->link == s) {
            ...
            for (; rel < rel_end; ++rel) {
                ...
                //if (n < 0) scc_error("internal: invalid symbol index in relocation");
...
ST_FUNC Section *new_section(SCCState *s1, const char *name, int sh_type, int sh_flags)
{
    ...
    sec = scc_mallocz(sizeof(Section) + SCC(strlen,int)(name));
    SCC(strcpy)(sec->name, name);
    sec->sh_type = sh_type;
...
ST_FUNC Section *new_symtab(SCCState *s1,
                           ...
                           const char *hash_name, int hash_sh_flags)
{
    ...
    ptr[0] = nb_buckets;
    ptr[1] = 1;
    SCC(memset)(ptr + 2, 0, (nb_buckets + 1) * sizeof(int));
    return symtab;
}
...
ST_FUNC void section_realloc(Section *sec, unsigned long new_size)
{
    ...
    SCC(memset)(data + sec->data_allocated, 0, size - sec->data_allocated);
    sec->data = data;
...
ST_FUNC Section *find_section(SCCState *s1, const char *name)
{
    ...
    for(i = 1; i < s1->nb_sections; i++) {
        sec = s1->sections[i];
        if (!SCC(strcmp,int)(name, sec->name))
            return sec;
    }
...
ST_FUNC int put_elf_str(Section *s, const char *sym)
{
    ...
    len = SCC(strlen,int)(sym) + 1;
    offset = s->data_offset;
    ptr = section_ptr_add(s, len);
    SCC(memmove)(ptr, sym, len);
    return offset;
}
...
static void rebuild_hash(Section *s, unsigned int nb_buckets)
{
    ...
    ptr += 2;
    hash = ptr;
    SCC(memset)(hash, 0, (nb_buckets + 1) * sizeof(int));
    ptr += nb_buckets + 1;
                            ...
                            int type, int symbol, addr_t addend)
{
    ...
    if (!sr) {
        ...
        SCC(snprintf)(buf, sizeof(buf), REL_SECTION_FMT, s->name);
        /* if the symtab is allocated, then we consider the relocation
           are also */
        sr = new_section(scc_state, buf, SHT_RELX, symtab->sh_flags);
...
#else
    if (addend)
        scc_error("non-zero addend on REL architecture");
#endif
}
...
ST_FUNC struct sym_attr *get_sym_attr(SCCState *s1, int index, int alloc)
{
    ...
    if (index >= s1->nb_sym_attrs) {
        ...
        s1->sym_attrs = tab;
        SCC(memset)(s1->sym_attrs + s1->nb_sym_attrs, 0,
...
/* In an ELF file symbol table, the local symbols must appear below
   the global and weak ones. Since SCC cannot sort it while generating
...
static void sort_syms(SCCState *s1, Section *s)
{
    ...
    nb_syms = s->data_offset / sizeof(ElfW(Sym));
    new_syms = scc_malloc(nb_syms * sizeof(ElfW(Sym)));
    ...
    SCC(memcpy)(s->data, new_syms, nb_syms * sizeof(ElfW(Sym)));
    scc_free(new_syms);
                                       ...
                                       int info, int sym_index)
{
    ...
    if (need_plt_entry) {
        ...
        len = SCC(strlen,int)(name);
        if (len > sizeof plt_name - 5)
            len = sizeof plt_name - 5;
        SCC(memcpy)(plt_name, name, len);
        SCC(strcpy)(plt_name + len, "@plt");
...
ST_FUNC void build_got_entries(SCCState *s1)
{
    ...
    for(i = 1; i < s1->nb_sections; i++) {
        ...
        for_each_elem(s, 0, rel, ElfW_Rel) {
	       ...
	       probably created by scc_add_symbol, and thus on 64-bit
	       targets might be too far from application code.  */
            if (gotplt_entry == AUTO_GOTPLT_ENTRY) {
                if (sym->st_shndx == SHN_UNDEF) {
                    ElfW(Sym) *esym;
		    int dynindex;
                    if (s1->output_type == SCC_OUTPUT_DLL && ! PCRELATIVE_DLLPLT)
                        continue;
		    /* Relocations for UNDEF symbols would normally need
		       ...
		       But SCC doesn't do that (at least for exes), so we
...
static void add_init_array_defines(SCCState *s1, const char *section_name)
{
    ...
    SCC(snprintf)(sym_start, sizeof(sym_start), "__%s_start", section_name + 1);
    SCC(snprintf)(sym_end, sizeof(sym_end), "__%s_end", section_name + 1);
...
{
    char buf[1024];
    SCC(snprintf)(buf, sizeof(buf), "%s/%s", s1->scc_lib_path, filename);
...
/* add scc runtime libraries */
//@ref elf_output_file(),
//sccrun.c::scc_relocate_ex(),
//sccpe.c::pe_output_file()
ST_FUNC void scc_add_runtime(SCCState *s1)
{
    ...
    if (!s1->nostdlib) {
        ...
        if (!s1->static_link) {
            if (SCC_LIBGCC[0] == '/')
                scc_add_file(s1, SCC_LIBGCC);
            else
                scc_add_dll(s1, SCC_LIBGCC, 0);
        }
#endif
...
static void scc_add_linker_symbols(SCCState *s1)
{
    ...
    for(i = 1; i < s1->nb_sections; i++) {
        ...
        if (s->sh_type == SHT_PROGBITS &&
            ...
            SCC(snprintf)(buf, sizeof(buf), "__start_%s", s->name);
            set_elf_sym(symtab_section,
                        0, 0,
                        ELFW(ST_INFO)(STB_GLOBAL, STT_NOTYPE), 0,
                        s->sh_num, buf);
            SCC(snprintf)(buf, sizeof(buf), "__stop_%s", s->name);
            set_elf_sym(symtab_section,
                        s->data_offset, 0,
                        ELFW(ST_INFO)(STB_GLOBAL, STT_NOTYPE), 0,
                        s->sh_num, buf);
                              ...
                              const int *sec_order)
{
    ...
    for(i=1;i<s1->nb_sections;i++) {
        ...
        if (s->sh_type != SHT_NOBITS &&
            ...
            size = s->sh_size;
            SCC(fwrite)(s->data, 1, size, f);
            offset += size;
        }
    }
}

ST_FUNC void fill_got_entry(SCCState *s1, ElfW_Rel *rel)
{
...
ST_FUNC void fill_got(SCCState *s1)
{
    Section *s;
    ElfW_Rel *rel;
    int i;
...
static int layout_sections(SCCState *s1, ElfW(Phdr) *phdr, int phnum,
        ...
        for(j = 0; j < 2; j++) {
            ...
            for(k = 0; k < 5; k++) {
                for(i = 1; i < s1->nb_sections; i++) {
                    ...
                    if (s->sh_type == SHT_RELX) {
#if defined(__FreeBSD__) || defined(__FreeBSD_kernel__)
                        if (!SCC(strcmp,int)(strsec->data + s->sh_name, ".rel.got")) {
                            dyninf->rel_addr = addr;
                            dyninf->rel_size += s->sh_size; /* XXX only first rel. */
                        }
                        if (!SCC(strcmp,int)(strsec->data + s->sh_name, ".rel.bss")) {
...
static void scc_output_elf(SCCState *s1, FILE *f, int phnum, ElfW(Phdr) *phdr,
                           int file_offset, int *sec_order)
{
    ...
    if (file_type == SCC_OUTPUT_EXE || file_type == SCC_OUTPUT_DLL)
    ...
    SCC(fwrite)(&ehdr, 1, sizeof(ElfW(Ehdr)), f);
    SCC(fwrite)(phdr, 1, phnum * sizeof(ElfW(Phdr)), f);
    ...
    for(i = 0; i < s1->nb_sections; i++) {
        sh = &shdr;
        SCC(memset)(sh, 0, sizeof(ElfW(Shdr)));
        s = s1->sections[i];
        if (s) {
        ...
        SCC(fwrite)(sh, 1, sizeof(ElfW(Shdr)), f);
    }
}
                              ...
                              ElfW(Phdr) *phdr, int file_offset, int *sec_order)
{
	...
	//file_type = s1->output_type;
	//if (file_type == SCC_OUTPUT_OBJ)
	//    mode = 0666;
	//else
	//    mode = 0777;
	SCC(unlink)(filename);

	////fd = ((SCC_DL_dlsym_int_func_ptr)scc_dlsym("open"))(filename, O_WRONLY | O_CREAT | O_TRUNC | O_BINARY, mode);
	//fd = ((SCC_DL_dlsym_int_func_ptr)scc_dlsym("open"))(filename, O_WRONLY | O_CREAT | O_TRUNC | O_BINARY, mode);
	//if (fd < 0) {
	//    scc_error_noabort("scc_write_elf_file(): could not write '%s',%d", filename,fd);
	//    return -1;
	//}
	//f = SCC(fdopen)(fd, "wb");

	f = SCC(fopen,FILE*)(filename,"wb");
	if (s1->verbose){
		SCC(printf)("<- %s\n", filename);
	}
	...
	file_type = s1->output_type;
	SCC(chmod)(filename, (file_type == SCC_OUTPUT_OBJ)?0666:0777);
	return 0;
}
...
static int elf_output_file(SCCState *s1, const char *filename)
{
	...
	resolve_common_syms(s1);
        ...
        if (!s1->static_link) {
            if (file_type == SCC_OUTPUT_EXE) {
                char *ptr;
                /* allow override the dynamic loader */
                const char *elfinterp = SCC(getenv)("LD_SO");
                if (elfinterp == NULL) elfinterp = DEFAULT_ELFINTERP(s1);
                /* add interpreter section only if executable */
                interp = new_section(s1, ".interp", SHT_PROGBITS, SHF_ALLOC);
                interp->sh_addralign = 1;
                ptr = section_ptr_add(interp, 1 + SCC(strlen,int)(elfinterp));
                SCC(strcpy)(ptr, elfinterp);
            }
    ...
    if (file_type == SCC_OUTPUT_OBJ)
        phnum = 0;
    else if (file_type == SCC_OUTPUT_DLL)
        phnum = 3;
    ...
    phdr = scc_mallocz(phnum * sizeof(ElfW(Phdr)));
    ...
    if (file_type != SCC_OUTPUT_OBJ) {
        ...
        if (dynamic) {
            ...
            if (file_type == SCC_OUTPUT_EXE
                || (RELOCATE_DLLPLT && file_type == SCC_OUTPUT_DLL))
                relocate_plt(s1);
	...
	tidy_section_headers(s1, sec_order);
        ...
        /* Perform relocation to GOT or PLT entries */
        if (file_type == SCC_OUTPUT_EXE && s1->static_link)
            fill_got(s1);
        else if (s1->got)
...
static void *load_data(int fd, unsigned long file_offset, unsigned long size)
{
    ...
    SCC(lseek)(fd, file_offset, SEEK_SET);
    SCC(read)(fd, data, size);
    return data;
}
...
//TODO macho
ST_FUNC int scc_load_object_file(SCCState *s1,
                                int fd, unsigned long file_offset)
{
    ...
    for(i = 1; i < ehdr.e_shnum; i++) {
        ...
        if (sh->sh_type != SHT_PROGBITS &&
            ...
            SCC(strcmp,int)(sh_name, ".stabstr")
            )
            continue;
	if (seencompressed
	    && (!SCC(strncmp,int)(sh_name, ".debug_", sizeof(".debug_")-1)
		|| (sh->sh_type == SHT_RELX
		    && !SCC(strncmp,int)((char*)strsec + shdr[sh->sh_info].sh_name,
	  ...
	  continue;
        ...
        for(j = 1; j < s1->nb_sections;j++) {
            s = s1->sections[j];
            if (!SCC(strcmp,int)(s->name, sh_name)) {
                if (!SCC(strncmp,int)(sh_name, ".gnu.linkonce",
    ...
    found:
        ...
        if (0 == SCC(strcmp,int)(sh_name, ".stab")) {
            stab_index = i;
            goto no_align;
        }
        if (0 == SCC(strcmp,int)(sh_name, ".stabstr")) {
            stabstr_index = i;
            goto no_align;
        }
    ...
    no_align:
        ...
        if (sh->sh_type != SHT_NOBITS) {
            unsigned char *ptr;
            SCC(lseek)(fd, file_offset + sh->sh_offset, SEEK_SET);
            ptr = section_ptr_add(s, size);
            SCC(read)(fd, ptr, size);
        } else {
            s->data_offset += size;
        }
    next: ;
...
static int scc_load_alacarte(SCCState *s1, int fd, int size, int entrysize)
{
    ...
    data = scc_malloc(size);
    if (SCC(read,int)(fd, data, size) != size)
        goto fail;
    ...
    do {
        bound = 0;
        for(p = ar_names, i = 0; i < nsyms; i++, p += SCC(strlen,int)(p)+1) {
...
ST_FUNC int scc_load_dll(SCCState *s1, int fd, const char *filename, int level)
{
    ...
    for(i = 0; i < s1->nb_loaded_dlls; i++) {
        ...
        if (!SCC(strcmp,int)(soname, dllref->name)) {
    ...
    dllref = scc_mallocz(sizeof(DLLReference) + SCC(strlen,int)(soname));
    dllref->level = level;
    SCC(strcpy)(dllref->name, soname);
    ...
    for(i = 0, dt = dynamic; i < nb_dts; i++, dt++) {
        ...
        case DT_NEEDED:
            ...
            for(j = 0; j < s1->nb_loaded_dlls; j++) {
                ...
                if (!SCC(strcmp,int)(name, dllref->name))
                    goto already_loaded;
            }
            if (scc_add_dll(s1, name, AFF_REFERENCED_DLL) < 0) {
...
static int ld_add_file_list(SCCState *s1, const char *cmd, int as_needed)
{
    ...
    group = !SCC(strcmp,int)(cmd, "GROUP");
    if (!as_needed)
        new_undef_syms();
    ...
    for(;;) {
        ...
        if (t == LD_TOK_EOF) {
            ...
            if (s1->static_link) {
                SCC(snprintf)(filename, sizeof filename, "lib%s.a", libname);
            } else {
                SCC(snprintf)(filename, sizeof filename, "lib%s.so", libname);
            }
        ...
        if (!SCC(strcmp,int)(filename, "AS_NEEDED")) {
...
ST_FUNC int scc_load_ldscript(SCCState *s1)
{
    ...
    for(;;) {
        ...
        if (t == LD_TOK_EOF)
            ...
            return -1;
        if (!SCC(strcmp,int)(cmd, "INPUT") ||
            !SCC(strcmp,int)(cmd, "GROUP")) {
            ret = ld_add_file_list(s1, cmd, 0);
            if (ret)
                return ret;
        } else if (!SCC(strcmp,int)(cmd, "OUTPUT_FORMAT") ||
                   !SCC(strcmp,int)(cmd, "TARGET")) {
