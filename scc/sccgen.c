...
ST_FUNC void scc_debug_funcstart(SCCState *s1, Sym *sym)
{
    ...
    SCC(snprintf)(buf, sizeof(buf), "%s:%c1",
    ...
    if (!sym->c) {
        ...
        if (sym_type == STT_FUNC && sym->type.ref) {
            ...
            if (ref->f.func_call == FUNC_STDCALL && can_add_underscore) {
                SCC(sprintf)(buf1, "_%s@%d", name, ref->f.func_args * PTR_SIZE);
                name = buf1;
                other |= ST_PE_STDCALL;
                can_add_underscore = 0;
            }
...
static Sym *__sym_malloc(void)
{
    ...
    sym_pool = scc_malloc(SYM_POOL_NB * sizeof(Sym));
...
static void patch_type(Sym *sym, CType *type)
{
    if (!(type->t & VT_EXTERN)) {
        if (!(sym->type.t & VT_EXTERN))
            scc_error("redefinition of '%s'", get_tok_str(sym->v, NULL));
    ...
    if (!is_compatible_types(&sym->type, type)) {
        ...
        if ((sym->type.t & VT_ARRAY) && type->ref->c >= 0) {
            ...
            if (sym->type.ref->c < 0)
                ...
                scc_error("conflicting type for '%s'", get_tok_str(sym->v, NULL));
        }
...
static void check_comparison_pointer_types(SValue *p1, SValue *p2, int op)
{
    ...
    invalid_operands:
        scc_error("invalid operands to binary %s", get_tok_str(op, NULL));
    }
                 ...
                 CType *type, const char *varstr)
{
    ...
    buf_size -= SCC(strlen,int)(buf);
    buf += SCC(strlen,int)(buf);
    ...
    case VT_PTR:
        s = type->ref;
        if (t & VT_ARRAY) {
            SCC(snprintf)(buf1, sizeof(buf1), "%s[%d]", varstr ? varstr : "", s->c);
            type_to_str(buf, buf_size, &s->type, buf1);
            goto no_var;
        }
...
redo:
	...
	case TOK_VISIBILITY2:
	    ...
	    if (!SCC(strcmp,int)(astr.data, "default"))
	        ad->a.visibility = STV_DEFAULT;
	    else if (!SCC(strcmp,int)(astr.data, "hidden"))
	        ad->a.visibility = STV_HIDDEN;
	    else if (!SCC(strcmp,int)(astr.data, "internal"))
	        ad->a.visibility = STV_INTERNAL;
	    else if (!SCC(strcmp,int)(astr.data, "protected"))
        ...
        case TOK_UNUSED1:
        case TOK_UNUSED2:
            /* currently, no need to handle it because scc does not
        ...
        default:
            if (scc_state->warn_unsupported)
                scc_warning("'%s' attribute ignored", get_tok_str(t, NULL));
            /* skip parameters */
            if (tok == '(') {
                int parenthesis = 0;
                do {
...
#ifdef BF_DEBUG
    SCC(printf)("struct size %-2d align %-2d\n\n", c, a);
    ...
    if (tok != '{') {
        ...
        if (s && (s->sym_scope == local_scope || tok != '{')) {
            ...
            if (u == VT_ENUM && IS_ENUM(s->type.t))
                goto do_decl;
            scc_error("redefinition of '%s'", get_tok_str(v, NULL));
        }
    } else {
        v = anon_sym++;
    }
...
static int parse_btype(CType *type, AttributeDef *ad)
{
    ...
    SCC(memset)(ad, 0, sizeof(AttributeDef));
    type_found = 0;
    typespec_found = 0;
    t = VT_INT;
    bt = st = -1;
			...
			for(;;) {
				...
				*plast = s;
				...
				if (l == FUNC_NEW && !parse_btype(&pt, &ad1)){
					...
					//scc_error("invalid type for '%s'", get_tok_str(v, NULL));
				}
			}
		} else
 ...
 push_tokc:
	...
	parse_builtin_params(0, "ee");
        ...
        if (r == VT_LLOCAL)
            r = VT_LOCAL;
        if (r != VT_LOCAL)
            scc_error("__builtin_va_start expects a local variable");
        vtop->r = r;
	vtop->type = char_pointer_type;
	vtop->c.i += 8;
	vstore();
...
special_math_val:
    ...
    while (1) {
	    ...
	    s = find_field(&vtop->type, tok);
            if (!s)
                scc_error("field not found: %s",  get_tok_str(tok & ~SYM_FIELD, &tokc));
...
static void block(int *bsym, int *csym, int is_expr)
{
	...
	nocode_wanted = saved_nocode_wanted;
        ...
        /* case lookup */
        gsym(b);
        SCC(qsort)(sw.p, sw.n, sizeof(void*), case_cmp);
        for (b = 1; b < sw.n; b++)
	    ...
	    next();
            ...
            if (s) {
                if (s->r == LABEL_DEFINED)
                    scc_error("duplicate label '%s'", get_tok_str(s->v, NULL));
                gsym(s->jnext);
                s->r = LABEL_DEFINED;
            } else {
...
static void init_putv(CType *type, Section *sec, unsigned long c)
{
	...
	if ((vtop->r & (VT_SYM|VT_CONST)) == (VT_SYM|VT_CONST) &&
	    ...
	    SCC(memmove)(ptr, ssec->data + esym->st_value, size);
	    if (ssec->reloc) {
...
#if defined SCC_IS_NATIVE_387
                if (sizeof (long double) >= 10) /* zero pad ten-byte LD */
                    SCC(memcpy)(ptr, &vtop->c.ld, 10);
//#ifdef __SAOCC__
            ...
            while (tok == TOK_STR || tok == TOK_LSTR) {
                ...
                if (!size_only) {
                    ...
                    if (sec && tok == TOK_STR && size1 == 1) {
                        if (!NODATA_WANTED)
                            SCC(memcpy)(sec->data + c + len, tokc.str.data, nb);
                    } else {
                        for(i=0;i<nb;i++) {
                            if (tok == TOK_STR)
		...
		while (1) { /* iterate thru each declaration */
			...
			if (tok == '{') {
				...
				if ((type.t & (VT_INLINE | VT_STATIC)) == 
					...
					fn = scc_malloc(sizeof *fn + SCC(strlen,int)(filename));
					SCC(strcpy)(fn->filename, filename);
					fn->sym = sym;
...
found:
					...
					sym->f = ad.f;
						...
						&& !(type.t & VT_EXTERN)) {
					scc_error("declaration of void object:%s",get_tok_str(v, NULL));
				} else {
					r = 0;
					if ((type.t & VT_BTYPE) == VT_FUNC) {
